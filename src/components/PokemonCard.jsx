...
export function PokemonCard({ name, sprite, types, detail }) {
    ...
    const handleCursor = detail ? { cursor: '' } : { cursor: 'pointer', width: '300px' }
    const size = detail ? { width: '500px', height: '300px' } : { width: '100%', height: '9rem' }
    ...
    return (
        <div >
            <div className="bg-gray-200 rounded-md flex flex-col gap-4 "
                style={handleCursor} onClick={() => handleNavigate(name)}>
                <h2 className="text-2xl text-center"> {name[0].toUpperCase() + name.substring(1)} </h2>
                <img style={size} src={sprite} alt={`Pokemon: ${name}`} />
                <div className="flex justify-center">
                    {types.map((data, index) => {
                        const { name } = data.type
                        return (
                            <span className="text-md m-2 font-semibold p-2 rounded-md"
                                style={{ backgroundColor: backgroundColor[name] }}
                                key={index}>{name[0].toUpperCase() + name.substring(1)}
                            </span>)
                    })}
                </div>
            </div>
